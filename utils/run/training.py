...
from utils.metrics.metrics import image_dice, metrics_from_confmat
...
def train_model(
    ...
    class_names = [None] * config.get("image").get("num_classes")
    tb = None
    ...
    with open (config.get("dirs").get("mapping"), 'r') as f:
        ...
        class_names = [class_id_mapping_image[i] for i in range(len(class_id_mapping_image))]
        tb = TensorboardLogger(
            enabled=True,                                   
            log_dir=tb_log_dir,
            num_classes=len(class_id_mapping_image),
            class_names=class_names,
            palette_rgb=color_mapping_image,               
            max_images=4,                                   
            log_every_steps=max(1, steps_train or 50),      
            denorm=None,                                   
            save_png_dir=os.path.join(save_dir, "tb_png")  
        )
    ...
    for epoch in range(init_epoch, num_epochs):
        ...
        for step, (x_img, y_img) in enumerate(train_dataloader):            
            ...
            (_, train_micro_miou, _) =\
                    metrics_from_confmat(confmat=batch_cm)
            
            global_step_train = epoch * num_steps_train + step
            if tb is not None:
                tb.log_step_scalars("train", epoch, step, global_step_train, loss=loss.item(), dice=float(img_dice), acc=float(train_micro_acc), miou=float(train_micro_miou))
        

            if step == 0 and train_first_batch is None:
                train_first_batch = (x_img.detach().cpu(), y_img.detach().cpu())
        ...
        with torch.no_grad():
            for step, (x_img, y_img) in tqdm(enumerate(val_dataloader), total=len(val_dataloader)):
                ...
                micro_acc_step = None
                micro_miou_step = None
                (_, micro_acc_step, _), (_, micro_miou_step, _) = metrics_from_confmat(confmat=batch_cm)

                if tb is not None:
                    global_step_val = epoch * len(val_dataloader) + step
                    tb.log_step_scalars("val", epoch, step, global_step_val, loss=loss.item(), dice=float(img_dice), acc=float(micro_acc_step), miou=float(micro_miou_step))

                if step == 0 and val_first_batch is None:
                    val_first_batch = (x_img.detach().cpu(), y_img.detach().cpu())
        ...
        if tb is not None:
            # 1) metrics loss/acc/miou/dice
            tb.log_epoch_metrics(
                epoch=epoch,
                train_loss=float(train_loss),
                val_loss=float(val_loss),
                train_micro_acc=float(train_micro_acc),
                val_micro_acc=float(val_micro_acc),
                train_micro_miou=float(train_micro_miou),
                val_micro_miou=float(val_micro_miou),
                train_dice=float(train_img_dice_mean),
                val_dice=float(val_img_dice_mean),
            )

            # 2) conf matrixes normalized
            tb.log_confmat(epoch, train_confmat, split="train")
            tb.log_confmat(epoch, val_confmat,   split="val")

            # 3) mosaics (img | mas | pred)
            """if train_first_batch is not None:
                tb.log_mosaic_from_batch(epoch, "train", train_first_batch, model=model, device=device)
            if val_first_batch is not None:
                tb.log_mosaic_from_batch(epoch, "val", val_first_batch, model=model, device=device)"""
        ...
        if tb is not None:
            train_png = os.path.join(save_image_path, f"train_{epoch}.png")
            val_png   = os.path.join(save_image_path,   f"val_{epoch}.png")
            tb.log_image_file(epoch, "train", train_png) 
            tb.log_image_file(epoch, "val",   val_png)  
